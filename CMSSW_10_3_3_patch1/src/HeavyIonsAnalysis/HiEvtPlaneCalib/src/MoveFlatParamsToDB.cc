...
private:
  ...
  virtual void endJob() ;
  bool GetRescor(int bin, string epname, double * res, double * eres);
...
// member functions
//
bool MoveFlatParamsToDB::GetRescor(int bin, string epname, double * restmp, double * erestmp) {
  FILE * ftest;
  ftest = fopen(Form("RescorTables/%s_%d.dat",epname.data(),bin),"r");
  if(ftest) {
    cout<<Form("RescorTables/%s_%d.dat added",epname.data(),bin)<<endl;
  }  else {
    std::cout<<"Failed to open: "<<Form("RescorTables/%s_%d.dat added",epname.data(),bin)<<endl;
    return kFALSE;
  }
  int nbins = 0;
  char buf[80];
...
void MoveFlatParamsToDB::AddCentralityBins( double centbin, int nbins) {
    ...
    cout<<"Add centbin "<<centbin<<"\t"<<nbins<<endl;
	...
	thisBin->RPNameIndx[i] = RPNameIndx[i];
	cout<<EPNames[RPNameIndx[i]]<<"\t"<<j<<"\t"<<thisBin->x[i]<<endl;
      } 
      rpFlat->m_table.push_back(*thisBin);
    }
  } 
...
MoveFlatParamsToDB::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for(int i = 0; i<nRP; i++) {
    if(GetRescor(1,EPNames[i],res1[i],eres1[i])) cent1 = kTRUE;
    if(GetRescor(2,EPNames[i],res2[i],eres2[i])) cent2 = kTRUE;
    if(GetRescor(5,EPNames[i],res5[i],eres5[i])) cent5 = kTRUE;
    if(GetRescor(10,EPNames[i],res10[i],eres10[i])) cent10 = kTRUE;
    if(GetRescor(20,EPNames[i],res20[i],eres20[i])) cent20 = kTRUE;
    if(GetRescor(25,EPNames[i],res25[i],eres25[i])) cent25 = kTRUE;
    if(GetRescor(30,EPNames[i],res30[i],eres30[i])) cent30 = kTRUE;
    if(GetRescor(40,EPNames[i],res40[i],eres40[i])) cent40 = kTRUE;
  }
