...
void EPCalib(unsigned int minRun=0, unsigned int maxRun=500000, string ssave="/rfs/sanders/tmpPbPb", string epsave="/rfs/sanders/EPPbPb.root",string foffsave="/rfs/sanders/foff.root"){
  ...
  TFile * foff = new TFile(foffsave.data(),"recreate");
  foff->cd();
  for(int i = 0; i<mx; i++) {
    for(int k = 0; k<flatnvtxbins_; k++){
      for(int j = 1; j<=7; j++){
	offs[k].wqxtrk[j-1][i]->Divide(offs[k].wqcnt[i]);
	offs[k].wqytrk[j-1][i]->Divide(offs[k].wqcnt[i]);
	offs[k].wqxtrk[j-1][i]->Write();
	offs[k].wqytrk[j-1][i]->Write();
      }
      offs[k].wqcnt[i]->Write();
    }
  }
